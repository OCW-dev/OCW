...
function transfer(txData, seed) {
    ...
    const wallet = WalletContractV4_1.WalletContractV4.create({ workchain: 0, publicKey: Buffer.from(publicK) });
...
async function signMultiTransaction(privateKey, messages, seqno, expireAt, workchain, publicKey) {
    if (!expireAt) {
    ...
    const wallet = WalletContractV4_1.WalletContractV4.create({ workchain: workchain == 1 ? 1 : 0, publicKey: Buffer.from(publicK) });
