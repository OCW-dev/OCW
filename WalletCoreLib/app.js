...
async function run(){
  ...
  console.log("welcome to ocw!\n\n");
    ...
    // console.log(ethWalletCreds.privateKey);
    ...
    console.log(account_addresses.data[0].addresses[0].address);

    let toAddr = '0xfda662b8cfa1c2edfbeeeffd62b628f90ce24133';

    let txSigningInfo = await wallet.getSigningInfo('1', account_addresses.data[0].addresses[0].address, toAddr, '123000000000000', timestamp_isos);
    console.log(txSigningInfo.data[0]);

    let txSing = await wallet.buildTx(account_addresses.data[0].addresses[0].address, ethWallet, toAddr, ethWalletCreds.privateKey);
    console.log(txSing);

    let broadcastTx = await wallet.broadcastTx(txSing, accountId, '1', account_addresses.data[0].addresses[0].address, timestamp_isos);
